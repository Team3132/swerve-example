...
import frc.robot.lib.log.LogWriter;
...
public class Chart implements Executable {
    ...
    private static void initChartFile(String csvColumns) {
        ...
        String nextFile = String.format("chart_%05d", (LogFileNumber.get() + 1));
        String previousFile = String.format("chart_%05d", (LogFileNumber.get() - 1));
        chartHTMLWriter.write(String.format("<html>\n" + "<head><title>%1$s plot.ly chart</title>\n"
                ...
                + "window.location.href =  \"http://\" + window.location.hostname + \":5801\" }\n"
                + "</script>\n"
                ...
                + "<br><br>\n"
                + "<a href='/%4$s/latest/Latest_log.txt'>Latest log</a>\n"
                + "<br><br>\n"
                + "<a href='/%4$s/data/%5$s.html'>Pervious file</a>\n"
                + "<br><br>\n"
                + "<a href='/%4$s/data/%6$s.html'>Next file</a>\n"
                + "<br><br>\n"
                + "<button onclick='redirecttoconfig()'>Config Server</button>\n"
                + "</body>\n", title, file, csvColumns, RobotName.get(), previousFile, nextFile));
